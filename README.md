...
**卡种颜色 + 占比条形统计**（2026-08-11 加，用了 `dataviz` skill 校验过的默认八色分类板）：
- 明细表格每行按卡名有不同的底色（卡名前也有一个同色小圆点），方便一眼看出同一行是哪种卡
- 表格上方有一条"条形统计"：把当前筛选出的记录按卡种分组，画成一条100%堆叠横条，每个色块宽度对应占比，
  色块下方拉一根竖线（相邻色块的线交替长短错开，避免窄色块的标签挤在一起）指到"卡名 百分比"文字
- **颜色是按"全量数据里出现频率"固定分配一次的，不是按当前筛选的子集重新排**：最常见的前8种卡各占一个
  分类色槽（按频率从高到低对应色板第1~8个颜色），排第9及以后的全部归进统一灰色的"其他"——这样切换筛选
  日期/搜索词的时候，同一张卡的颜色不会跳变（dataviz skill 的规则：颜色要跟着"这是哪张卡"走，不能跟着
  "这次筛选里排第几"走）。回兴店目前全量有21种卡名，超过8种的部分会被"其他"吸收
- 配色是 `dataviz` skill 提供的默认分类色板（8色+灰色其他），跑过 `validate_palette.js` 全部通过
