# cross-store-rate

回兴店跨店率每日结算 + 可视化看板。每天北京时间 10:00 通过 GitHub Actions 自动结算"前一天"的数据，
结果通过 GitHub Pages 展示成一个带日期区间选择的动态曲线图，也支持本地双击 `index.html` 直接看。

跟 `booking-fn` 是分开的独立小项目，不占用它在 Vercel 上宝贵的12个函数名额，纯粹调用
`booking-fn` 已有的接口，没有自己的后端。

## 跨店率怎么算的

跨店率 = 分子 ÷ 分母，算法在 `lib.js` 里，`collect.js`（每日结算）和 `backfill.js`（历史回填）共用同一份，
保证两边数字口径一致，不会因为算法不一样出现断层。

- **分母**：回兴店当天持有有效套餐的会员数（按手机号去重）。**不用会员列表接口**
  （`GET /api/users`，实测经常 500 报错，不稳定），改用"购买记录"(`purchases.js` 默认模式) +
  "验券记录"(`purchases.js?mode=exchange-records`) 反推出每张卡的有效区间：
  `[生效日期, 失效日期]` = `[orderCreateTime, userCardExpiredTime]`。
  某天落在区间内就算这个用户当天有效，一天的分母 = 当天有至少一张卡覆盖的用户数。
  ⚠️ **这是近似值**：如果一张卡在到期日之前就被用完了，`userCardExpiredTime` 不会跟着更新提前，
  这张卡会被算成"比实际有效期更长"，分母会略微偏高——2026-08-06 已跟用户确认接受这个精度
  （精确算法要挨个查每张失效卡的使用记录定位真实失效时间，量级在千张以上、跑起来太慢）。
  ⚠️ **查这两个接口必须传 `startTime`/`endTime` 日期范围，不能留空**——王知之的
  `exchange-records` 接口不传日期范围时会默默截断成一小段最近记录，不是真的"不限时间"，
  2026-08-06 踩过这个坑（一度被截断到只有32/78/99条这种不稳定的小数字，传了正确日期范围后才稳定在
  2300+条）。
  ⚠️ **冻结的卡、锁座固定座位的卡都不算"有效"**（2026-08-11 用户明确要求）：`lib.js` 的
  `isExcludedFromValid()` 会把 `userCardStatus === "FREEZE"`（冻结，跟 VALID/INVALID 并列的独立状态）
  或者 `cardTypeCode === "LONG_TERM"`（`cardTypeName` 显示"锁座卡"，也就是锁座固定座位那类）的记录整条
  剔除，不参与任何一天的有效区间计算——回兴店全量 2557 条记录里有 42 条命中（9条冻结+33条锁座），
  剔除后这些记录既不会出现在"有效套餐用户明细"里，也不会计入跨店率的分母。这条规则改了以后用
  `recompute-denominator.js` 把 `history.json` 里已有的全部历史记录重新按新口径算了一遍（分子不受影响，
  原样保留，只有分母/跨店率变了），436天历史记录里有405天的分母发生了变化（基本每天都有零星的冻结/锁座卡）。
- **分子**：当天(北京时间)跨店结算**流出方向**(`purchases.js?mode=cross-store-settlement&direction=out`)
  涉及的用户数，按手机号去重——也就是"回兴店卖出的卡，当天在别的门店消费"的人数，这个是精确值不是近似值。

## 锁座用户怎么算的（2026-08-12 加）

看板第三条曲线"锁座用户趋势"，跟"明细数据"表格最后一列"锁座用户"是同一个指标：**当天有"锁座订单"覆盖的
用户数**（按手机号去重）。

⚠️ **判断口径是订单本身的时间跨度，不是卡种**——跟前面"分母"那节说的"锁座固定座位的卡不算有效"
完全是两回事，别搞混：那个看的是**这张卡本身是不是锁座卡**（`cardTypeCode === "LONG_TERM"`），
这个看的是**这一笔订座订单订的时间跨度是不是超过3天**，任何卡种（哪怕是普通天卡/小时卡）只要一笔订单
跨度超过3天就算一次锁座订单——`lib.js` 的 `isLockedSeatOrder()`：

- 用回兴店全量订座历史（`orders.js`，不是 `purchases.js`），单笔订单 `[startTime日期, endTime日期]`
  相差**超过3天**（`daysBetweenDates() > 3`）就算锁座订单
- 已取消的订单（orderStatus 4/9）不算，跟"当日状态"的"本店预定"同一个排除逻辑——取消了不代表真的占用座位
- 每个用户可能有多笔锁座订单，`buildLockedSeatIntervals()` 按手机号整理出这些订单各自的 `[开始,结束]`
  区间，某天落在任意一个区间内就算这个用户当天"锁座"，算法上跟"分母"那套区间判断逻辑是同一个套路
  （`lockedSeatUsersOnDate()` 对应 `denominatorOnDate()`）

⚠️ **这块要多拉一次回兴店全量订座历史（1.6万+条，服务端分页上限100条/页，要拉160+页）才能算**，
`collect.js` 每天都要多花2~3分钟拉这份数据，比加这个指标之前明显慢了——不影响功能，只是定时任务
跑起来的耗时变长了，GitHub Actions 单个 job 默认超时6小时，完全够用。

## 文件结构

- `lib.js`：共享逻辑（拉数据、算分母/分子/锁座用户），`collect.js`/`backfill.js` 都靠它
- `collect.js`：每日结算脚本，算出"前一天"一条记录，追加/覆盖进 `history.json`
- `backfill.js`：一次性历史回填脚本，见下面单独一节
- `recompute-denominator.js`：分母算法改了以后，用新口径重新计算 `history.json` 里已有全部记录的分母/跨店率
  （分子不变）。跟 `backfill.js` 不一样——`backfill.js` 只补没有的日期，这个是覆盖已有的日期。
  平时不用跑，只有改了 `lib.js` 分母算法之后才需要跑一次：`FUNC_SHARED_SECRET=xxx node recompute-denominator.js`
- `recompute-locked-seats.js`：新加指标专用的一次性补算脚本，把 `history.json` 里已有的全部历史记录
  补上 `lockedSeatUsers` 字段（其他字段不变）。以后再加新指标，照这个套路再写一个新的 recompute 脚本就行，
  不用改现有的这两个：`FUNC_SHARED_SECRET=xxx node recompute-locked-seats.js`
- `history.json`：累积的每日数据 `[{date, numerator, denominator, rate, lockedSeatUsers, backfilled?}, ...]`，
  `backfilled: true` 表示这条是回填出来的近似值，不是真实每日结算跑出来的
- `history.js`：内容跟 `history.json` 完全一样，只是包了一层 `window.HISTORY_DATA = [...]`——
  `index.html` 用 `<script src="./history.js">` 加载数据而不是 `fetch('./history.json')`，
  因为本地双击打开网页时 `fetch` 读同目录文件会被浏览器的本地文件 CORS 限制挡住、显示不出数据，
  `<script src>` 没有这个限制，两种打开方式都能用（2026-08-06 发现这个问题后改的）
- `index.html`：看板页面，跨店率+有效套餐数量两条动态曲线图 + 日期区间选择器 + 明细表格 +
  "有效套餐用户明细"（本地专用，见下一节）。**聚合部分（曲线图/KPI/明细表格）只有数字，不含任何手机号/昵称**，
  可以放心公开
- `.github/workflows/collect.yml`：每天定时跑 `collect.js`，提交 `history.json`/`history.js`，再部署到 GitHub Pages

### 同一份 `index.html` 里的本地专用板块：有效套餐用户明细（含手机号，不进公开仓库）

`index.html` 最下面"有效套餐用户明细"卡片可以查某天具体是哪些用户持有有效套餐（手机号、昵称、卡名、
生效/失效日期），**这部分没有单独做一个 HTML 文件**，而是巧妙合并进同一份 `index.html` 里：

- `gen-local-detail.js`：本地跑一次，拉全部购买+验券记录，生成 `local-detail-data.js`
- `local-detail-data.js`：含真实客户手机号，已加入 `.gitignore`，**不会被提交，线上仓库里也没有这个文件**
- `index.html` 用 `<script src="./local-detail-data.js">` 去加载这份数据——**线上 GitHub Pages 上这个文件根本不存在**，
  这个 `<script>` 请求会静默 404（不报错、不崩页面），"有效套餐用户明细"卡片自动显示"本地未生成数据"的提示；
  **只有在本地跑过生成脚本之后，这个文件才会真实存在于你电脑上**，双击打开 `index.html` 才能看到完整明细表格
  （支持选日期、手机号/昵称模糊搜索、默认手机号打码显示，点按钮切明文）。2026-08-11 验证过这个降级逻辑：
  临时删掉本地数据文件重新打开页面，明细卡片正常显示提示文案，没有报错也没有露出任何数据

**卡种颜色 + 占比条形统计**（2026-08-11 加，配色用了 `dataviz` skill 校验过的默认八色分类板）：
- 明细表格每行按卡名有不同的底色（卡名前也有一个同色小圆点），方便一眼看出同一行是哪种卡
- 表格上方有一条"条形统计"：把当前筛选（查看日期+搜索）出的记录按卡种分组，画成一条100%堆叠横条，
  **从左到右按数量从多到少排列**，每个色块宽度对应占比，色块够宽的话百分比直接写在色块正中间；
  色块下方拉一根竖线（相邻色块的线交替长短错开，避免窄色块的标签挤在一起）指到"卡名 (具体数量)"文字
- **所有卡名都单独展示，不折叠成"其他"**（2026-08-11 用户明确要求）：最常见的前8种卡用 `dataviz` skill
  校验过的默认分类色板，第9种及以后用固定公式（黄金角哈希色相）生成额外颜色，保证每种卡都有自己独立的
  颜色。回兴店目前全量有21种卡名，都能各自显示
- **颜色是按"全量数据里出现频率"固定分配一次的，不是按当前筛选的子集重新排**：这样切换筛选日期/搜索词、
  或者点色块筛选的时候，同一张卡的颜色不会跳变（dataviz skill 的规则：颜色要跟着"这是哪张卡"走，不能跟着
  "这次筛选里排第几"走）；条形统计里色块从左到右的**排列顺序**倒是按当前这次筛选的数量重新排的，
  这个跟颜色分配是两回事，颜色固定、位置动态
- **点击某个色块可以筛选下方明细表格**，只显示这个卡种的记录，KPI 数字也会跟着变，上方出现一个
  "筛选中：卡名 ×" 的提示条；再点一次同一个色块，或者点提示条上的 × 可以取消筛选。条形统计本身
  **不会**跟着"点色块选中的卡种"变化，还是按查看日期+搜索显示完整占比分布，方便切换着看别的卡种占比

**"当日状态"列**（2026-08-11 加）：明细表格最后一列，把这个人在"查看日期"当天的活跃程度跟
跨店结算记录、回兴店本店订座记录做交叉比对：
- 当天在跨店结算(流出方向)记录里出现（这个人当天在别的门店消费了）→ 显示"跨店预定"
- 当天在回兴店本店订座记录里出现（排除掉已取消的单）→ 显示"本店预定"
- 两边当天都没有 → 往前找这个人最近一次(跨店或本店，不限)有记录的日期，算出相差天数，
  显示"已闲置N天"；如果这个人从来没有任何跨店/本店记录，显示"无历史订座记录"

**当日状态占比条形统计**（2026-08-11 加，在卡种占比条形统计下方）：把上面三种状态归成三个固定颜色的
色块——**绿色=本店预定，红色=跨店预定，灰色=闲置**（"已闲置N天"和"无历史订座记录"都算灰色的"闲置"）。
跟卡种条形统计不一样，这三个颜色是**写死的固定色**（复用分类色板的绿/红/灰槽位），不是按频率分配的，
因为状态本身就只有三种、含义也是固定的，不需要动态分配。点色块一样可以筛选下方表格，而且**可以跟
卡种筛选同时生效**（AND 关系，比如同时选中"天卡"+"跨店预定"，只看天卡持有者里当天跨店消费的那几个人）——
两条条形统计上各自独立显示自己的"筛选中：xxx ×"提示条，两个筛选互不影响、可以分别单独清除。

**点了当日状态色块之后，下面会多冒出第三条条形统计**（2026-08-11 加）：限定在选中的那个状态范围内，
再画一条"这个状态下卡种占比"——比如点了"闲置"，就能看到闲置的这些人主要是哪些卡种（"是不是某种卡的人
特别容易闲置"这类问题一眼就能看出来）。不选中任何状态时这条不出现。这条卡种条形统计跟最上面那条卡种
条形统计**共用同一个筛选状态**（点哪条的色块都行，两条会同步高亮），复用的是同一份 `renderCardComposition`
函数，只是传了不同的挂载点 id。

这块要额外拉两份数据（`lib.js` 的 `buildActivityDates()`）：跨店结算历史（974条，很快）+
回兴店本店全量订座历史（1.6万+条，服务端分页上限100条/页，要拉160+页，**明显更慢**，
`gen-local-detail.js` 现在整体要跑2~3分钟，比之前只拉卡记录慢很多，正常现象，耐心等）。
⚠️ **踩过一个分页参数名的坑**：`orders.js` 接口的翻页参数是 `pageNo`，不是更常见的 `currentPage`——
一开始传错成 `currentPage`，`orders.js` 内部读不到就一直默认第1页，16016条订单实际全被拉成同一页
数据重复161次拼出来的"假16016条"（真实distinct id只有100个），算出来的活跃用户数少得离谱
（只有46个，明显不对），改成 `pageNo` 后才拉到真实的16016条不重复记录、涉及1595个不同手机号。

## 锁座订单明细（2026-08-12 加，本地专用，含手机号）

跟公开看板"锁座用户趋势"图是同一个判定口径（单笔订座订单跨度超过3天算锁座订单，已取消的不算），
但这个是详细列出**具体是谁定的、从几号定到几号、是代客下单还是小程序自己下单**，含手机号，
只能放本地专用工具，不能进公开仓库——跟"有效套餐用户明细"是同一套隐私处理方式。

- 表格列：手机号(默认打码)/昵称/座位号/开始日期/结束日期/跨度天数/支付方式(原始代码)/预定方式
- **预定方式怎么判断**：靠订单的 `paymentMethod` 字段（支付方式）。⚠️ **这是真实抓包验证过的，
  不是猜的**：这个项目开发过程中用 `book.js`（代客下单接口）实际下过的单子，查回来
  `paymentMethod` 全部是 `"6"`；同一个客户更早自己在小程序上订的单子是 `"9"`。全量锁座订单里
  `paymentMethod` 只出现过 `"6"`（66条）和 `"8"`（17条），从没出现过普通订单常见的
  `"9"/"3"/"4"/"5"`——`"8"` 没有单独验证过具体含义，但行为特征（只出现在锁座订单里、payAmt常为0）
  跟 `"6"` 高度相似，一并归进"代客下单锁座"这一类，对应用户说的"支付方式里面有线下结算"。
  这个映射写在 `lib.js` 的 `OFFLINE_PAYMENT_METHODS`/`classifyLockedOrderBookingType()`，
  以后如果锁座订单出现新的支付方式代码，要重新核实分类是否还准确
- `booking-fn` 的 `orders.js` 原来没有透传 `paymentMethod` 字段，2026-08-12 加上了（只是把上游已有的
  字段原样多传一个出来，没改查询逻辑，不影响其他调用方）
- 页面上也有一条"预定方式占比"条形统计（代客下单锁座=紫，小程序下单锁座=蓝，避开跟"当日状态"
  绿/红/灰撞色），点色块一样能筛选表格，用法跟前面几条条形统计是同一套交互
- ⚠️ **这块跟"当日状态"共用同一份订座原始数据**（都是 `lib.js` 的 `fetchStoreOrders()`），
  `gen-local-detail.js` 2026-08-12 改成只拉一次共用，不然两边各自拉一次要多等一倍时间

用法：

```bash
FUNC_SHARED_SECRET=xxx node gen-local-detail.js
```

跑完刷新（或重新打开）本地的 `index.html` 即可看到明细。想看最新数据，重新跑一遍生成脚本再刷新页面就行——
这个工具没有自动定时更新，需要手动跑。

## 配置

GitHub 仓库 Settings → Secrets and variables → Actions 加一个 secret：

| Secret 名 | 值 |
|---|---|
| `FUNC_SHARED_SECRET` | `booking-fn` 的 `x-func-secret`，在 `../booking-fn/README.md` 里 |

Settings → Pages，Source 选 "GitHub Actions"（workflow 里已经配好了部署步骤，选完就行）。

监控哪个门店，改 `lib.js` 顶部的 `STORE` 常量（默认 `"回兴"`），门店关键词规则跟 `booking-fn` 一样。

## 历史回填

`collect.js` 只会从它第一次跑的那天开始往后累积。如果想把更早的历史数据也补上：

```bash
FUNC_SHARED_SECRET=xxx node backfill.js
```

- 回填的时间范围写死在 `lib.js` 的 `EARLIEST_DATE` 常量里（现在是 `2025-06-01`，回兴店用户要求的起点）
- 只回填 `history.json` 里**还没有**的日期，不会覆盖 `collect.js` 正式跑出来的真实记录
- 只回填到"昨天"，跟每日结算的口径一致（今天的数据还没"结算完"）
- 这是一次性重活（要拉全量购买+验券+跨店结算历史数据，两千多条，跑下来大概1分钟），补完一次以后不用再跑，
  除非想扩大 `EARLIEST_DATE` 往更早补

## 数据口径的几个说明

- **同一天重复跑会覆盖**，不会重复累加——如果一天内手动触发了好几次 workflow，`history.json` 里这天永远只有最新一次跑出来的结果
- **验券记录查全部渠道**（`channel: ''`），不是只查美团——2026-08-06 发现不传渠道限制不影响准确性，干脆查全部更完整
- ⚠️ **接口报错时会直接中止整个脚本，不会提交半成品数据**：王知之后台偶尔会抽风返回
  `{"error":"fetch failed"}`（这个大项目从头到尾都踩过好几次），`lib.js` 的 `fetchAllPages`
  遇到这种报错会自动重试3次，3次都失败就直接抛异常让整个脚本失败退出——2026-08-10 吃过一次亏，
  当时遇到报错只是打印日志、拿着不完整数据（购买记录被拦腰截断、验券记录变成0条）硬算，
  结果算出"跨店率130%"这种明显错误的数字还提交上去了，用户第二天发现才补救。现在遇到这种情况，
  workflow 会直接跑失败（GitHub 上能看到红叉），`history.json` 保持不变，等下一次定时/手动触发再试。
- ⚠️ **跨店结算数据当天可能还没同步完整**：2026-08-08 那天 workflow 曾在北京时间 00:58 跑过，
  接口没报错、正常返回但只有 0 条，好几天后再查才是真实的 14 条——不是接口报错（那种情况会重试+中止），
  是数据本身当时就还没结算同步完，无法靠代码检测。已把定时从 00:30 挪到 10:00 留缓冲，但具体要多久才能
  稳定同步完还不确定，以后如果再发现某天的数字偏低，可以手动用 `mode=cross-store-settlement` 接口重新
  查一遍核对，不一致就手动改 `history.json` 对应那天的记录再提交。
- ⚠️ **本地打开的 `index.html` 不会自己联网同步**：这是个纯静态文件，双击打开显示的是"你电脑上这份文件
  当时的内容"，不会因为 GitHub 上的数据更新了就跟着变。**想看最新数据，去线上 Pages 地址**
  （见仓库 About 里的链接），或者本地 `git pull` 一下再重新打开本地文件。
- ⚠️ **看到的最新日期永远比"今天"晚一天是正常的，不是 bug**：定时任务北京时间10:00才跑，算的是
  "昨天"（跨店结算数据要过凌晨才结算完，见上面"跨店率怎么算的"那节）。比如今天是8.13，10:00之前
  看到的最新数据还是8.11（因为8.12的数据要等今天10:00那次跑才会算出来），10:00之后才会变成8.12。
  2026-08-13 用户反馈过一次"数据好像停在8.10了"，实际排查是浏览器缓存了旧版本——线上数据其实已经
  更新到8.11了（用 `curl`/直接 `fetch` 绕过缓存查证实的），刷新时如果怀疑缓存卡住了可以强制刷新
  （Mac上 Cmd+Shift+R）。

## 记住上次选的选项（2026-08-13 加）

页面里的日期区间/预设按钮/搜索词/打码开关/点色块选中的筛选，都会自动存到浏览器的 `localStorage` 里，
下次打开自动恢复成上次的样子，不用每次重新选。存的东西都是些界面偏好（日期范围选了哪天、搜不搜了什么
关键词之类），**不含任何实际数据**，跟公开仓库/隐私没关系。

- 存取都包了一层 try/catch，`localStorage` 不可用（比如浏览器隐私模式禁用了它）会静默降级成"不记住"，
  不影响页面正常使用
- 每个浏览器/每台设备是分开记的（`localStorage` 本来就是按浏览器+域名隔离的），换个电脑打开不会带过去
- 涉及的 key 都带 `crossStoreRate.` 前缀，浏览器开发者工具里搜这个前缀能看到全部存了什么

## 手动触发测试

去 GitHub 仓库 Actions 页面，选这个 workflow，点 "Run workflow" 立刻跑一次（会重新结算"前一天"这一条）。

## 本地测试

```bash
FUNC_SHARED_SECRET=xxx node collect.js
```

⚠️ 本地跑会直接改本地的 `history.json`/`history.js`，测完注意别把测试数据误提交上去
（`git checkout -- history.json history.js` 可以丢弃本地改动）。
